...
Long tasks return compact progress by default, then the answer when finished. Turn `completed` is Grok ending the turn, not confirmed tool outcomes or independent validation. Detailed tool history and full text remain available on demand. See [results and cancellation](docs/usage.md#results-and-cancellation) and [efficiency measurements](docs/efficiency.md).
