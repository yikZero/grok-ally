...
| Cancellation? | [`acp_agent.rs`](https://github.com/xai-org/grok-build/blob/72a61251fcffb464bcc687aeb5a998e5a98ec0c9/crates/codegen/xai-grok-shell/src/agent/mvp_agent/acp_agent.rs#L2166) implements ACP `session/cancel`. Live Grok leaves background terminals running after that ACK; `session/close` is what reclaims them. The bridge sends cancel, then close, then retires the process so a follow-up loads the same `sessionId`. |
...
Bounded request handles solve host tool deadlines without a daemon or a job database. Prompts are not retried after ambiguous failures. Grok owns conversation persistence; the bridge keeps active connections and recent tool summaries in memory. Assistant text and sanitized tool-state history are stored in private temporary files, paged through `grok_status`, and removed on eviction or shutdown. Revision waits wake on observed progress; they do not start another model request. Same-session exclusion is currently within one MCP process, so multi-host use requires an explicit handoff after disconnecting the original client.
