...
5. Run `npm pack --ignore-scripts --pack-destination <release-directory>`. In that directory, generate `SHA256SUMS` with `shasum -a 256 <package.tgz> > SHA256SUMS`.
6. Create and push the `v<version>` tag at the tested commit. Publish with `gh release create v<version> <package.tgz> SHA256SUMS --verify-tag --title "v<version>" --notes-file /tmp/grok-bridge-release.md`.
